    ...
    <div>
        <div v-if="showClearListModal" @click="showClearListModal = false"
            ...
            <div class="w-[90%] max-w-lg bg-background-950 rounded-xl p-5 relative">
                <X class="absolute top-5 right-5 cursor-pointer" size="26" @click="showClearListModal = false" title="close window"/>
                ...
                <div class="flex flex-col gap-5" >
                    ...
                    }" class="w-full h-14 bg-primary-950 rounded-lg transition font-semibold" title="remove all articles">Remove all
                        articles</button>
                    <button @click="() => {
                        store.clear(true)
                        showClearListModal = false
                    }" class="w-full h-14 bg-primary-600 rounded-lg transition font-semibold" title="remove only read articles">Remove only
