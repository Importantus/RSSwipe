...
<template>
  <div>
    ...
    >
      <div class="w-[90%] max-w-lg bg-background-950 rounded-xl p-5 relative">
        ...
        <form @submit.prevent="store.deleteUser(deletionPassword)" class="flex flex-col gap-3">
          ...
          <button
            type="submit"
            class="w-full h-14 bg-red-500 rounded-lg hover:bg-red-600 transition"
          >
            Delete Account
          </button>
        </form>
    ...
    <div class="px-5 overflow-y-scroll pb-10 relative z-10">
      <TitleNavigationBar title="Account" backNavigationPath="/" />

      <div v-if="store.error" class="w-full bg-red-500 rounded-lg p-3 z-10">{{ store.error }}</div>
      <div v-if="store.success" class="w-full bg-green-500 rounded-lg p-3 z-10">
        {{ store.success }}
      </div>

      <h2 class="mb-5 mt-10 text-xl">Update Data</h2>
      <form @submit.prevent="store.updateUserDetails" class="flex flex-col gap-3">
        ...
        <button
          type="submit"
          class="w-full h-14 bg-amber-600 rounded-lg hover:bg-amber-700 transition"
        >
          Update Details
        </button>
      </form>
      ...
      <form
        @submit.prevent="store.updateUserPassword(password, oldPassword)"
      ...
      >
        ...
        />
        <button
          :disabled="password.length < 8 || oldPassword.length < 8"
          type="submit"
